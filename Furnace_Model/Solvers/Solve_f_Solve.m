function [T_f_new, T_a_new, T_w_new, T_ms_new, T_h_new] = ...
    Solve_f_Solve(T_f_old, T_a_old, T_w_old, T_ms_old, T_h_old, ...
                  fm, Q_power, k, T_f_older, T_a_older, T_w_older, T_ms_older, T_h_older)
    ...
    T_vec = fsolve(@(T) Residual_Wrapper(T, T_f_old, T_a_old, T_w_old, T_ms_old, T_h_old, Q_power, fm, k), ...
                    T_guess, options);
    ...
    T_ms_new = T_vec(3);
    T_h_new = T_vec(4);
    T_w_new = FVM_Wall_Solver(T_w_old, T_f_new, T_h_new, fm, k);
end
