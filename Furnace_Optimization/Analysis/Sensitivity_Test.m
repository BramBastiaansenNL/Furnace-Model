function J_matrix = Sensitivity_Test(furnace_model, constraints, Temp_range, t_range, N)
    ...
    for i = 1:length(Temp_range)
        for j = 1:length(t_range)
            ...
            % Evaluate cost
            try
                J_matrix(i, j) = Compute_Cost_Function(x, furnace_model, constraints, N);
                feasible_mask(i, j) = Check_Feasibility(x, constraints, furnace_model, N);
            catch
                J_matrix(i, j) = NaN;
    ...
    % Plot 3D surface
    Plot_3D_Cost_Surface(t_range, Temp_range, J_matrix, 'Log10 Cost Function Sensitivity');
