function mech_loss = Compute_Mechanical_Loss(M, furnace_model)
    ...
    loss_YS  = w_YS * ( max(-delta_YS, 0).^2 - reward_gain * max(delta_YS, 0).^2 );
    loss_UTS = w_UTS * ( max(-delta_UTS,0).^2 - reward_gain * max(delta_UTS,0).^2 );
    loss_UE  = w_UE * ( max(-delta_UE,0).^2 - reward_gain * max(delta_UE,0).^2 );
