function [T_simulation, fm] = Simulate_Set_Furnace(T_f_desired, t, fm)
    %% Implicit Numerical Solver for 1D Heat Diffusion Equation
    
    % Update furnace model
    fm.settings.desired_temperature_curve = true;
    fm = Update_Furnace_Model(fm, t, fm.model.N);
    ...
    for k = 1:Nt-1
        ...
        [Power_curve(k), integral_error, prev_error, fm] = ...
            Compute_Power_Supply(T_f_desired(k), T_furnace(k), integral_error, ...
                                 prev_error, dt, fm, k);
        ...
        % Checks whether the alloy is a lumped mass or distribution
        if fm.settings.alloy_temperature_distribution
            % Pass the entire column vector at time k
            T_alloy_current = T_alloy(:, k);
            if k > 1
                T_alloy_old = T_alloy(:, k-1);
            end
        else
            % Pass the scalar
            T_alloy_current = T_alloy(k);
        ...
        if k == 1
            % First step
            [T_furnace(k+1), T_alloy_next, T_walls(:, :, k+1), T_metal_sheet(k+1), T_heater(k+1), ~, fm] = ...
                Newton_Raphson_Iteration(T_furnace(k), T_alloy_current, T_walls(:, :, k), T_metal_sheet(k), T_heater(k), fm, Power_curve(k), k+1);
       
        else
            % Subsequent steps extrapolate initial guesses
            [T_furnace(k+1), T_alloy_next, T_walls(:, :, k+1), T_metal_sheet(k+1), T_heater(k+1), ~, fm] = ...
                Newton_Raphson_Iteration(T_furnace(k), T_alloy_current, T_walls(:, :, k), T_metal_sheet(k), T_heater(k), ...
                fm, Power_curve(k), k+1, T_furnace(k-1), T_alloy_old, T_walls(:, :, k-1), T_metal_sheet(k-1), T_heater(k-1));
        end

        % Store alloy temperature (either lumped or distributed)
        if fm.settings.alloy_temperature_distribution
            T_alloy(:, k+1) = T_alloy_next;   % vector
        else
            T_alloy(k+1) = T_alloy_next;     % scalar
        end
    end
