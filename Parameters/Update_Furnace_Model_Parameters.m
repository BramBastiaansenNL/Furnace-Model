function fm = Update_Furnace_Model_Parameters(parameters, fm, t)
    %% Updates furnace model with selected parameters
    %
    % parameters : vector of parameter values
    % fm.param_subset : corresponding cell array of param names (order matches vector)
    % fm : furnace model struct
    
    % Default parameters
    if isempty(fm.param_subset)
        % No update — return unchanged model
        return;
    end

    % Safety check: parameter vector size must match param_subset length
    if length(parameters) ~= length(fm.param_subset)
